 ...
 * @param {string|number} value The string to mask
 * @param config - The config for the mask
 */
export function maskMiddle(value: string | number,	config: { visibleDigits: number, maskChar: string, fixedMaskCharacterLength: number } = { visibleDigits: 2, maskChar: "*", fixedMaskCharacterLength: 4 }) {
  // 将输入转换为字符串
  const str = value.toString()
  const { visibleDigits, maskChar, fixedMaskCharacterLength } = config
  // 输入验证
  if (!str || str.length <= 2 * visibleDigits)
    return str
